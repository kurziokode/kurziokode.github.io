	...
	create: function( options ) {
		return (function(){
			...
			function setupWrapper() {
				...
				dom.wrapper.style[ Curz.prefix( 'perspective' ) ] = '800px';
				dom.wrapper.style[ Curz .prefix( 'perspectiveOrigin' ) ] = contentsTransformOrigin;
			}
			...
			function setupCover() {
				...
				if( supports3DTransforms ) {
					dom.cover.style[ Curz.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
			...
			function setupMenu() {
				...
				if( supports3DTransforms ) {
					style[ Curz.prefix( 'transform' ) ] = menuTransformClosed;
					style[ Curz.prefix( 'transformOrigin' ) ] = menuTransformOrigin;
					style[ Curz.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
				else {
					Curz.extend( style, menuStyleClosed );
				}
			}
			...
			function setupContents() {
				...
				if( supports3DTransforms ) {
					style[ Curz.prefix( 'transform' ) ] = contentsTransformClosed;
					style[ Curz.prefix( 'transformOrigin' ) ] = contentsTransformOrigin;
					style[ Curz.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
				else {
					style.position = style.position.match( /relative|absolute|fixed/gi ) ? style.position : 'relative';
					Curz.extend( style, contentsStyleClosed );
				}
			}
			...
			function bindEvents() {
				...
				if( 'ontouchstart' in window ) {
					if( config.touch ) {
						Curz.bindEvent( document, 'touchstart', onTouchStart );
						Curz.bindEvent( document, 'touchend', onTouchEnd );
					}
					else {
						Curz.unbindEvent( document, 'touchstart', onTouchStart );
						Curz.unbindEvent( document, 'touchend', onTouchEnd );
					}
				}

				if( config.mouse ) {
					Curz.bindEvent( document, 'mousedown', onMouseDown );
					Curz.bindEvent( document, 'mouseup', onMouseUp );
					Curz.bindEvent( document, 'mousemove', onMouseMove );
				}
				else {
					Curz.unbindEvent( document, 'mousedown', onMouseDown );
					Curz.unbindEvent( document, 'mouseup', onMouseUp );
					Curz.unbindEvent( document, 'mousemove', onMouseMove );
				}
			}
			...
			function open() {
				if( !isOpen ) {
					...
					Curz.addClass( dom.wrapper, 'curz-active' );
					...
					if( supports3DTransforms ) {
						...
						contentsAnimation = Curz.animate( dom.contents, contentsStyleOpened, 500 );
						coverAnimation && coverAnimation.stop();
						coverAnimation = Curz.animate( dom.cover, { opacity: 1 }, 500 );
					}

					Curz.dispatchEvent( dom.menu, 'open' );
				}
			}
			...
			function close() {
				if( isOpen ) {
					...
					if( supports3DTransforms ) {
						...
						contentsAnimation && contentsAnimation.stop();
						contentsAnimation = Curz.animate( dom.contents, contentsStyleClosed, 500 );
						coverAnimation && coverAnimation.stop();
						coverAnimation = Curz.animate( dom.cover, { opacity: 0 }, 500, function() {
							dom.cover.style.visibility = 'hidden';
							Curz.dispatchEvent( dom.menu, 'closed' );
						} );
					}
					Curz.dispatchEvent( dom.menu, 'close' );
				}
			}
			...
			function onTouchStart( event ) {
				...
				Curz.bindEvent( document, 'touchmove', onTouchMove );
			}
			...
			return {
				...
				removeEventListener: function( type, listener ) {
					dom.menu && Curz.unbindEvent( dom.menu, type, listener );
				}
			};
